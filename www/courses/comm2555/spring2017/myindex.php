<img src="images/textbooks2.jpg" alt="" 
    class="float-left" style="width:130px;padding:5px;">

<h3>About the Course</h3>
<aside class="aside1">
    <h5>Links</h5>
    <ul>
        <li><a href="syllabus-comm2555-spring2017.pdf">Syllabus</a>
        <li><a href="resources">Resources</a>
        <li><a href="https://bb9.uni.edu/">eLearning</a>
        <li><a href="http://ids.uni.edu/">ids.uni.edu</a>
    </ul>
</aside>

<div style="margin-left:160px;">
    <p>This course is one of the four core courses in the <a href="http://ids.uni.edu">Interactive Digital Studies (IDS) program</a>. It provides a foundation in digital production skills that students will need for all their other IDS classes. 

    <p>Specific content varies from semester to semester; we typically use Adobe Photoshop fot digital image editing and HTML & CSS for web page coding and design. Students will also learn about principles of visual design and designing for the web, as well as proper usage of digital images and other media online.
</div>


<br style="clear:all">

<h3>Schedule</h3>

<table class="schedule days-3">
	<thead>
		<tr>
			<th>Week
			<th>Date
			<th>Readings
			<th>Sessions &amp; Labs
			<th>Assignments
	<tbody>
		<tr class="week">
			<td rowspan="3">1
			<td>Mon, 01/09
			<td>
			<td><a href="sessions/1">Introduction to the course</a>
			<td>
		<tr>
			<td>Wed, 01/11
			<td>Duckett, Intro
			<td><a href="sessions/2.pdf">Introduction to the Internet and the WWW</a>
			<td>
		<tr>
			<td>Fri, 01/13
			<td>Duckett, ch. 1
			<td><a href="sessions/3.pdf">Building web pages / working with a server</a>
			<td>
		<tr class="week break">
			<td rowspan="3" style="background:#f1f1f1;">2
			<td>Mon, 01/16
			<td>
			<td colspan="2">Martin Luther King, Jr. Day
		<tr>
			<td>Wed, 01/18
			<td>
			<td>NO CLASS
			<td>Assignment 1<br><em>(available on eLearning)</em>
		<tr>
			<td>Fri, 01/20
			<td>
			<td><a href='sessions/4'>Introduction to HTML</a>
			<td>
		<tr class="week">
			<td rowspan="3">3
			<td>Mon, 01/23
			<td>Duckett, ch. 2
			<td><a href="sessions/5">HTML common elements; structure and semantics</a>
			<td>
		<tr>
			<td>Wed, 01/25
			<td>Duckett, ch. 3, 6
			<td><a href="sessions/6.pdf">HTML structured content: lists and tables</a>
			<td>
		<tr>
			<td>Fri, 01/27
			<td>
			<td class="lab"><a href="labs/lab1.pdf">Lab 1: Getting started with HTML</a> | <a href="labs/lab1.zip">lab files</a>
			<td>
		<tr class="week">
			<td rowspan="3">4
			<td>Mon, 01/30
			<td>Duckett, ch. 4
			<td>HTML links
			<td><a class="hw" href="homework/2.pdf">Assignment 2</a>
		<tr>
			<td>Wed, 02/01
			<td>Duckett, ch. 5
			<td>HTML images
			<td>
		<tr>
			<td>Fri, 02/03
			<td>
			<td class="lab"><a href="labs/lab2.pdf">Lab 2: HTML images and links</a>
				<br><a href="labs/lab2extra.pdf">Lab 2 - extra credit: HTML images and links + introducing CSS</a> | <a href="labs/lab2extra.zip">lab files</a>
			<td>
		<tr class="week">
			<td rowspan="3">5
			<td>Mon, 02/06
			<td>Duckett, 8, 10
			<td>Styling web pages with CSS
			<td>
		<tr>
			<td>Wed, 02/08
			<td>
			<td>CSS: how the cascade works; common selectors
			<td>
		<tr>
			<td>Fri, 02/10
			<td>
			<td class="lab"><a href="labs/lab3.pdf">Lab 3</a> | <a href="labs/lab3.zip">lab files</a>
			<td>
		<tr class="week">
			<td rowspan="3">6
			<td>Mon, 02/13
			<td>Duckett, ch. 11
			<td>CSS: styling color
			<td><a class="hw" href="homework/3.pdf">Assignment 3</a> | <a class="hw" href="homework/3/source.txt">source.txt</a>
		<tr>
			<td>Wed, 02/15
			<td>Duckett, ch. 12
			<td>CSS: styling text
			<td>
		<tr>
			<td>Fri, 02/17
			<td>
			<td class="lab"><a href="labs/lab4.pdf">Lab 4</a> | <a href="labs/lab4.zip">lab files</a>
			<td>
		<tr class="week">
			<td rowspan="3">7
			<td>Mon, 02/20
			<td>Duckett, ch. 13
			<td>CSS box model
			<td>
		<tr>
			<td>Wed, 02/22
			<td>Duckett, ch. 14
			<td>CSS: styling tables and lists
			<td>
		<tr>
			<td>Fri, 02/24
			<td>
			<td class="lab">Lab 5
			<td>
		<tr class="week">
			<td rowspan="3">8
			<td>Mon, 02/27
			<td>Duckett, ch. 15
			<td>Lab/box model review
			<td><a class="hw" href="homework/4.pdf">Assignment 4</a>
		<tr> 
			<td>Wed, 03/01
			<td>Duckett, ch. 16
			<td>CSS layout: floating elements
			<td>
		<tr>
			<td>Fri, 03/03
			<td>
			<td class="lab"><a href="labs/lab6.pdf">Lab 6</a> | <a href="labs/lab6.zip">lab files</a>
			<td>
		<tr class="week">
			<td rowspan="3">9
			<td>Mon, 03/06
			<td>
			<td>Exam review
			<td>
		<tr>
			<td>Wed, 03/08
			<td>
			<td>Exam 1
			<td>
		<tr>
			<td>Fri, 03/10
			<td>Lynda tutorials
			<td> 
			<td>

		<tr class="break">
			<td colspan="5">
		<tr class="break week">
			<td colspan="3" style="border-right-width:0; border-top-width:0">
			<td colspan="2" style="text-align:left">SPRING BREAK
		<tr class="break">
			<td colspan="5">

		<tr class="week">
			<td rowspan="3">10
			<td>Mon, 03/20
			<td><a href="http://www.nytimes.com/2006/06/26/arts/26crea.html">In Digital Age...</a>
			<td>Copyright, Public Domain, and the Creative Commons
			<td>
		<tr>
			<td>Wed, 03/22
			<td>Williams, ch. 1, 2, 3
			<td>Design principles: proximity and alignment; Intro to Photoshop 
			<td>
		<tr>
			<td>Fri, 03/24
			<td><a href="tutorials">Lynda tutorials</a>
			<td>Lab 7: Photoshop: layers and basic selection techniques
			<td>
		<tr class="week">
			<td rowspan="3">11
			<td>Mon, 03/27
			<td>Williams, ch. 4, 5
			<td>Design principles: repetition and contrast
			<td>
		<tr>
			<td>Wed, 03/29
			<td>Lynda tutorials
			<td>Photoshop: compositing, more selection techniques, smart objects
			<td>
		<tr>
			<td>Fri, 03/31
			<td><a href="tutorials">Lynda tutorials</a>
			<td>Lab 8: Photoshop: advanced selection techniques
			<td>
		<tr class="week">
			<td rowspan="3">12
			<td>Mon, 04/03
			<td>Williams, ch. 7
			<td>Designing with color
			<td>Assignment 5
		<tr>
			<td>Wed, 04/05
			<td>Williams, ch. 10-12
			<td>Typography
			<td>
		<tr>
			<td>Fri, 04/07
			<td><a href="tutorials">Lynda tutorials</a>
			<td>Lab 9: Photoshop: adjustment layers
			<td>
		<tr class="week">
			<td rowspan="3">13
			<td>Mon, 04/10
			<td>
			<td>Exam review
			<td>Assignment 6
		<tr>
			<td>Wed, 04/12
			<td>
			<td>Exam 2
			<td>
		<tr>
			<td>Fri, 04/14
			<td>
			<td>Lab 10: CSS review + elements of layout
			<td>
		<tr class="week">
			<td rowspan="3">14
			<td>Mon, 04/17
			<td>
			<td>Building web sites: web page structure
			<td>
		<tr>
			<td>Wed, 04/19
			<td>TBD
			<td>CSS: approaches to layout
			<td>
		<tr>
			<td>Fri, 04/21
			<td>
			<td>Lab 11: CSS layout
			<td>Assignment 7
		<tr class="week">
			<td rowspan="3">15
			<td>Mon, 04/24
			<td>TBD
			<td>Project implementation: layout
			<td>
		<tr>
			<td>Wed, 04/26
			<td>TBD
			<td>Project implementation: editorial style
			<td>
		<tr>
			<td>Fri, 04/28
			<td>
			<td>Project implementation: wrap-up
			<td>
		<tr class="week">
			<td rowspan="2">16
			<td>Mon, 05/01
			<td>
			<td>Section 2 Final Project Presentations: 3 - 4:50 pm
			<td>
		<tr>
			<td>Tue, 05/02
			<td>
			<td>Section 1 Final Project Presentations: 1 - 2:50 pm
			<td>
	</table>
